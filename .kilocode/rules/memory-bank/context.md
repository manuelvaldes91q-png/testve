...
| Mar 26, 2026 | **EWINET ping to localhost**: Changed EWINET ping to use `window.location` (same server running the app). Added `getPingUrl()` helper that returns `{protocol}//{host}/favicon.ico` for EWINET and static URLs for other destinations. |
