...
| Mar 26, 2026 | **Speedtest-style parallel connections**: Rewrote download/upload tests to use 4 parallel connections (like Ookla Speedtest's 4 threads). Download: 4 concurrent fetch streams with adaptive speed sampling, 10s duration, outlier removal (top 2 + bottom 25%). Upload: 4 concurrent threads sending 1MB chunks for 10s. CLI test updated with same parallel approach. |
