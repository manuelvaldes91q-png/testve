...
| Mar 26, 2026 | **Fixed inaccurate ping values**: `imgPing` used `fetch` with `mode: "no-cors"` which resolved before the actual HTTP response arrived, giving artificially low latency. Replaced with `<img>` tag approach that waits for full response. Updated Cloudflare targets to use `/cdn-cgi/trace` endpoint (~200 bytes) for more accurate RTT measurement. |
