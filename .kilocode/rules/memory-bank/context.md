...
| Mar 26, 2026 | **EWINET ping to own ISP**: Changed EWINET ping from `one.one.one.one` to `portal.ewinet.com` (user's own ISP infrastructure in Valencia). Note: EWINET portal only supports HTTP (no HTTPS), may cause mixed content issues on HTTPS deployments. |
