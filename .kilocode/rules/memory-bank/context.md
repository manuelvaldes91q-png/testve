...
| Mar 26, 2026 | **Fixed ping URLs - removed HTTP redirects**: `http://1.1.1.1` and `http://one.one.one.one` returned 301 redirect to HTTPS, adding an extra round trip to each ping measurement. Changed to direct HTTPS URLs: `https://1.1.1.1/cdn-cgi/trace` and `https://one.one.one.one/cdn-cgi/trace` (~200 bytes, no redirect). All 5 ping endpoints now verified working without redirects. |
| Mar 26, 2026 | **EWINET ping to Telemic Speedtest server**: Changed EWINET to use official Ookla Speedtest server for Valencia (Corporacion Telemic, server ID 38944): `https://speedtest.vcia.inter.com.ve:8080/speedtest/latency.txt` (9 bytes). HTTPS on port 8080. Added `getPingUrl()` helper for dynamic URL resolution. |
| Mar 26, 2026 | **ASN/BGP route identification**: Added `/api/asn` server route that resolves hostnames via Cloudflare DoH and looks up ASN info via ipinfo.io. Frontend loads ASN data for all 5 destinations on mount. Displays ASN number, organization, and IP address in a dedicated section below the latency grid. |
